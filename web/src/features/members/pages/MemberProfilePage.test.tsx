...
import { activeSubscription, cancelledRenewal, queuedRenewal, subscriptionsPage } from "@/test/subscriptions";
...
describe("MemberProfilePage", () => {
  ...
  it("Subscription_ActiveWithAQueuedRenewal_CanStillFreezeTheActiveOneFromItsRow", async () => {
    ...
    const api = mockApi({
      ...signedInHandlers(owner),
      [`GET /api/members/${reza.id}`]: () => json(200, reza),
      [`GET /api/members/${reza.id}/subscriptions`]: () => subscriptionsPage([queuedRenewal, activeSubscription]),
      [`POST /api/subscriptions/${activeSubscription.id}/freeze`]: () =>
        json(200, { ...activeSubscription, status: "Frozen", frozenSince: activeSubscription.startDate, version: 2 }),
    });
    renderApp(`/members/${reza.id}`, { session: session() });
    ...
    expect(api.requestsTo("POST", `/api/subscriptions/${activeSubscription.id}/freeze`)).toHaveLength(1);
  });
  ...
  it("Subscription_Refund_ExceedsNetPaid_ShowsThePersianError", async () => {
    ...
    fireEvent.change(screen.getByLabelText("مبلغ استرداد (تومان)"), { target: { value: "500000" } });
    ...
    expect(await screen.findByText("این استرداد از مبلغ پرداخت‌شدهٔ اشتراک بیشتر است.")).toBeInTheDocument();
  });
  ...
  it("History_SubscriptionAlreadyPaid_DisablesItsPaymentButton", async () => {
    const paidSubscription = { ...activeSubscription, netPaid: activeSubscription.price, paymentStatus: "Paid" as const };
    mockApi({
      ...signedInHandlers(staffUser),
      [`GET /api/members/${reza.id}`]: () => json(200, reza),
