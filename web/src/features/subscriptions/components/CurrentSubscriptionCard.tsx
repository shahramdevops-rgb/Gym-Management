      ...
      <CardContent className="space-y-4">
        ...
        {current === null ? (
          ...
          <dl className="grid gap-x-6 gap-y-3 text-sm sm:grid-cols-[max-content_1fr]">
            ...
            <dd>{current.totalSessions === null ? "نامحدود" : formatNumber(Number(current.remainingSessions))}</dd>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" variant="outline" disabled={current === null} onClick={() => togglePanel("renew")}>
            تمدید
          </Button>
        </div>

        {panel === "assign" && (
          <AssignSubscriptionForm memberId={memberId} onDone={() => done("اشتراک فروخته شد.")} onCancel={() => setPanel(null)} />
        )}
