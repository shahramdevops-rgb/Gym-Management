...
export function pickCurrentSubscription(items: Subscription[]): Subscription | null {
  ...
  if (upcoming.length > 0) {
    return upcoming.reduce((soonest, item) => (item.startDate < soonest.startDate ? item : soonest));
  }
...
export function useAssignSubscription() {
  return useSubscriptionMutation(async ({ memberId, planId }: { memberId: string; planId: string }) => {
